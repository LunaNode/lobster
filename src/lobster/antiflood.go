...
func antifloodAction(db *Database, ip string, action string) {
	...
	if rows.Next() {
		...
		db.Exec("INSERT INTO antiflood (ip, action, count, time) VALUES (?, ?, 0, NOW())", ip, action)
	}
}

func antifloodCheck(db *Database, ip string, action string, maxCount int) bool {
	var countBad int
	db.QueryRow("SELECT COUNT(*) FROM antiflood WHERE ip = ? AND action = ? AND count > ?", ip, action, maxCount).Scan(&countBad)
	return countBad == 0
}
